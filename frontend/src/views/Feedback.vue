<template>
  <v-container>
    <v-row>
      <v-col cols="12">
                ...
                <v-row>
                  <v-col cols="12" sm="6" md="3">
                    <div class="text-center">
                      <h3 class="text-h3 text-primary">{{ stats.totalCount }}</h3>
                  ...
                  <v-col cols="12" sm="6" md="3">
                    <div class="text-center">
                      <h3 class="text-h3 text-amber-darken-2">{{ stats.avgScore }}</h3>
                  ...
                  <v-col cols="12" sm="6" md="3">
                    <div class="text-center">
                      <h3 class="text-h3 text-green-darken-1">{{ stats.withCommentCount }}</h3>
                  ...
                  <v-col cols="12" sm="6" md="3">
                    <div class="text-center">
                      <h3 class="text-h3 text-blue-darken-1">{{ stats.withCommentPercent }}%</h3>
        ...
        <v-row v-if="feedbackList && feedbackList.length > 0">
          <v-col cols="12">
            <v-card v-for="feedback in feedbackList" :key="feedback.id" class="mb-4">
              <v-card-title class="d-flex align-center">
                <span>{{ userStore.isAdmin ? `用户ID: ${feedback.user_id}` : '我的反馈' }}</span>
                <v-spacer></v-spacer>
                <v-chip
                  color="amber"
                  size="small"
              ...
              <v-card-subtitle>
                ...
                <span class="text-caption ml-2 text-grey">{{ formatDateTime(feedback.updated_at) }}</span>
              </v-card-subtitle>
    ...
    <AppDialog
      ...
      title="编辑反馈"
      :subtitle="editingFeedback ? `景点: ${editingFeedback.attraction_name} 用户ID: ${editingFeedback.user_id} 时间: ${formatDateTime(editingFeedback.updated_at)}` : ''"
      confirmText="更新"
      :loading="loading"
      :hideActions="true"
...
const loadFeedback = async () => {
  ...
  try {
    ...
    if (filters.attraction_id !== null) {
      params.attraction_id = typeof filters.attraction_id === 'object' ? 
        (filters.attraction_id as any).id : filters.attraction_id
    }
