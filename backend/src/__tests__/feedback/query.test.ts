  ...
  describe('POST /api/v1/feedback/query', () => {
    ...
    it('普通用户应该只能查询自己的反馈', async () => {
      ...
      const allBelongToUser = response.body.data.feedback.every(
        (item: any) => item.user_id === regularUser.id && item.status === 'public'
      );
    ...
    it('普通用户应该能查询特定景点的所有反馈', async () => {
      ...
      expect(response.body.data.feedback.every((item: any) => 
        item.attraction_id === testAttractions[0].id && item.status === 'public'
      )).toBe(true);
    });
    ...
    it('用户应该能获取自己对特定景点的反馈', async () => {
      ...
      const userFeedback = response.body.data.feedback.find(
        (item: any) => item.user_id === regularUser.id && item.attraction_id === testAttractions[0].id
      );
    ...
    it('用户应该能直接查询自己对特定景点的单个反馈', async () => {
      // 先清除测试数据，确保只有一个反馈
      await Feedback.destroy({ where: {} });
      const testFeedback = await createTestFeedback(regularUser.id, testAttractions[0].id);
      ...
      expect(response.body.data.feedback[0].attraction_id).toBe(testAttractions[0].id);
