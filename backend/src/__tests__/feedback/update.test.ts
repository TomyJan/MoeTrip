...
describe('反馈模块-更新反馈', () => {
  ...
  describe('POST /api/v1/feedback/update', () => {
    it('用户应该能更新自己的反馈 (通过ID)', async () => {
      // 创建测试反馈
      const feedback = await createTestFeedback(regularUser.id, testAttractions[0].id);
      ...
      expect(updateResponse.body.data.feedback.comment).toBe(updateData.comment);
    ...
    it('用户不应该能修改他人的反馈', async () => {
      // 创建管理员的测试反馈
      const adminFeedback = await createTestFeedback(adminUser.id, testAttractions[0].id);
